...
class CController
{
	...
	int initLBMSolver() {
		...
		cLbmPtr = new CLbmSolver<T>(_UID, *cq_computation, *cq_communication,
				*cContext, *cDevice, _BC, _domain,
...
public:
	...
	void simulationStepAlpha_nocallback() {
#if DEBUG
		DEBUGPRINT("###### Simulation Step ALPHA NO CALLBACK ######\n" );
#endif

		// SIMULATION_STEP_ALPHA
		CVector<3, int> x_size(1, _domain.getSize()[1], _domain.getSize()[2]);
		CVector<3, int> y_size(_domain.getSize()[0] - 4, 1,
				_domain.getSize()[2]);
		CVector<3, int> z_size(_domain.getSize()[0] - 4,
				_domain.getSize()[1] - 4, 1);
		//
		//		    cl_int err;
		...
		cLbmPtr->simulationStepAlphaRect(x0_origin, x_size, 0, NULL, &ev_ss_x0);
		cLbmPtr->simulationStepAlphaRect(x1_origin, x_size, 1, &ev_ss_x0,
				&ev_ss_x1);
		...
		storeDataAlpha(MPI_COMM_DIRECTION_X_0, 1, &ev_ss_x0, &ev_store_x0);
		storeDataAlpha(MPI_COMM_DIRECTION_X_1, 1, &ev_ss_x1, &ev_store_x1);

		cLbmPtr->wait();
		cq_communication->finish();
		...
		cLbmPtr->simulationStepAlphaRect(y0_origin, y_size, 0, NULL, &ev_ss_y0);
		cLbmPtr->simulationStepAlphaRect(y1_origin, y_size, 1, &ev_ss_y0,
				&ev_ss_y1);
		...
		storeDataAlpha(MPI_COMM_DIRECTION_Y_0, 1, &ev_ss_y0, &ev_store_y0);
		storeDataAlpha(MPI_COMM_DIRECTION_Y_1, 1, &ev_ss_y1, &ev_store_y1);
		...
		cLbmPtr->simulationStepAlphaRect(z0_origin, z_size, 0, NULL, &ev_ss_z0);
		cLbmPtr->simulationStepAlphaRect(z1_origin, z_size, 1, &ev_ss_z0,
				&ev_ss_z1);

		// --> Store z boundary
		cl_event ev_store_z0;
		cl_event ev_store_z1;
		storeDataAlpha(MPI_COMM_DIRECTION_Z_0, 1, &ev_ss_z0, &ev_store_z0);
		storeDataAlpha(MPI_COMM_DIRECTION_Z_1, 1, &ev_ss_z1, &ev_store_z1);
	...
	void simulationStepBeta_nocallback() {
#if DEBUG
		DEBUGPRINT("###### Simulation Step BETA NO CALLBACK ######\n" );
#endif
		...
		CVector<3, int> x0_origin(1, 0, 0);
		CVector<3, int> x1_origin(_domain.getSize()[0] - 2, 0, 0);
		cLbmPtr->simulationStepBetaRect(x0_origin, x_size, 0, NULL, &ev_ss_x0);
		cLbmPtr->simulationStepBetaRect(x1_origin, x_size, 1, &ev_ss_x0,
				&ev_ss_x1);
		...
		storeDataBeta(MPI_COMM_DIRECTION_X_0, 1, &ev_ss_x0, &ev_store_x0);
		storeDataBeta(MPI_COMM_DIRECTION_X_1, 1, &ev_ss_x1, &ev_store_x1);
		...
		CVector<3, int> y0_origin(2, 1, 0);
		CVector<3, int> y1_origin(2, _domain.getSize()[1] - 2, 0);
		cLbmPtr->simulationStepBetaRect(y0_origin, y_size, 0, NULL, &ev_ss_y0);
		cLbmPtr->simulationStepBetaRect(y1_origin, y_size, 1, &ev_ss_y0,
				&ev_ss_y1);
		...
		cl_event ev_store_y0;
		cl_event ev_store_y1;
		storeDataBeta(MPI_COMM_DIRECTION_Y_0, 1, &ev_ss_y0, &ev_store_y0);
		storeDataBeta(MPI_COMM_DIRECTION_Y_1, 1, &ev_ss_y1, &ev_store_y1);
		...
		CVector<3, int> z0_origin(2, 2, 1);
		CVector<3, int> z1_origin(2, 2, _domain.getSize()[2] - 2);
		cLbmPtr->simulationStepBetaRect(z0_origin, z_size, 0, NULL, &ev_ss_z0);
		cLbmPtr->simulationStepBetaRect(z1_origin, z_size, 1, &ev_ss_z0,
				&ev_ss_z1);

		// --> Store z boundary
		cl_event ev_store_z0;
		cl_event ev_store_z1;
		storeDataBeta(MPI_COMM_DIRECTION_Z_0, 1, &ev_ss_z0, &ev_store_z0);
		storeDataBeta(MPI_COMM_DIRECTION_Z_1, 1, &ev_ss_z1, &ev_store_z1);
	...
	void computeNextStep() {
		...
		if (_simulation_step_counter & 1)
			simulationStepAlpha_nocallback();
		else
			simulationStepBeta_nocallback();
